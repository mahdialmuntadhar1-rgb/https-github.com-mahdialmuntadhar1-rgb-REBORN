...
const BusinessPostcard: React.FC<Props> = ({ business, onClick, onClaimClick }) => {
  ...
  return (
    <motion.div
      whileHover={{ y: -4 }}
      whileTap={{ scale: 0.98 }}
      className="flex flex-col bg-surface rounded-2xl overflow-hidden border border-border shadow-sm relative aspect-[4/5]"
    >
      {/* Top 60%: Cover Image */}
      ...
      <div className="relative h-[40%] p-3 pt-6 flex flex-col justify-between cursor-pointer" onClick={onClick}>
        ...
        <div className="absolute -top-6 inset-inline-start-3 w-12 h-12 rounded-full border-2 border-surface bg-surface overflow-hidden shadow-sm">
          {business.logoUrl ? (
        ...
        <div className="flex flex-col gap-0.5">
          <h5 className="text-xs font-bold truncate text-text-primary">{name}</h5>
        ...
        <div className="flex items-center justify-between mt-1">
          <div className="flex items-center gap-1">
            ...
            <span className="text-[10px] font-bold text-text-primary">{business.rating || 4.5}</span>
          </div>
