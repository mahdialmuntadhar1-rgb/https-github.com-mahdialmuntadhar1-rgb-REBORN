  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#0a0a0f] text-white">
      ...
      <div className="p-6 flex items-center gap-4 border-b border-white/5">
        <button onClick={prevStep} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white/60 hover:text-white transition-colors">
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-4">
        ...
        <button 
          onClick={onRefer}
          className="w-full py-5 bg-white/5 border border-white/10 rounded-2xl text-white font-bold text-sm hover:bg-white/10 transition-all flex items-center justify-center gap-3"
        >
          <Share2 size={20} />
        ...
        {methods.map(m => (
          ...
          >
            ...
            <div className="flex-1 flex flex-col gap-1">
              ...
              <span className="text-[10px] text-white/40 leading-tight">{m.desc}</span>
            </div>
            <ChevronRight size={20} className={`text-white/20 ${isRTL ? 'rotate-180' : ''}`} />
          </button>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest">{language === 'ar' ? 'اسم العمل (عربي)' : 'Business Name (AR)'}</label>
            <input 
              type="text" 
              defaultValue={business.nameAr}
              className="w-full h-14 bg-white/5 border border-white/10 rounded-2xl px-4 text-sm focus:outline-none focus:border-primary/50"
            />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest">{language === 'ar' ? 'اسم العمل (إنجليزي)' : 'Business Name (EN)'}</label>
            <input 
              type="text" 
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest">{language === 'ar' ? 'رقم الهاتف' : 'Phone Number'}</label>
            <div className="relative">
              <Phone className="absolute left-4 top-1/2 -translate-y-1/2 text-white/20" size={18} />
              <input 
                type="tel" 
                defaultValue={business.phone}
                className="w-full h-14 bg-white/5 border border-white/10 rounded-2xl pl-12 pr-4 text-sm focus:outline-none focus:border-primary/50"
              />
            </div>
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest">{language === 'ar' ? 'واتساب (اختياري)' : 'WhatsApp (Optional)'}</label>
            <div className="relative">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col gap-2">
            <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest">{language === 'ar' ? 'الشعار' : 'Logo'}</label>
            <button className="aspect-square bg-white/5 border border-dashed border-white/20 rounded-2xl flex flex-col items-center justify-center gap-2 text-white/20 hover:text-white/40 transition-colors">
              <Camera size={24} />
              <span className="text-[10px] font-bold">Upload</span>
            </button>
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest">{language === 'ar' ? 'صورة الغلاف' : 'Cover'}</label>
            <button className="aspect-square bg-white/5 border border-dashed border-white/20 rounded-2xl flex flex-col items-center justify-center gap-2 text-white/20 hover:text-white/40 transition-colors">
              <Upload size={24} />
              <span className="text-[10px] font-bold">Upload</span>
            </button>
...
function Step4({ onFinish, isRTL, language }: any) {
  return (
    <div className="flex flex-col items-center justify-center py-10 text-center gap-8">
      ...
      <button 
        onClick={onFinish}
        className="w-full py-5 bg-white/5 border border-white/10 rounded-2xl text-white font-bold text-sm hover:bg-white/10 transition-all mt-10"
      >
